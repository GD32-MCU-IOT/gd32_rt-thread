...
#ifdef BSP_I2C_DMA_TIMEOUT
#define I2C_DMA_TIMEOUT_MS      BSP_I2C_DMA_TIMEOUT
#else
...
static void gd32_i2c_dma_config_channel(struct dma_config *dma, uint32_t periph_addr, uint8_t *buf, uint16_t len, uint32_t direction)
{
    ...
    dma_deinit(dma->periph, dma->channel);
    ...
    dma_channel_subperipheral_select(dma->periph, dma->channel, dma->subperiph);
