...
| UART           |     支持     | UART0 - UART5                                                 |
| LED            |     支持     | LED1（PC0），LED2（PC2），LED3（PE0），LED4（PE1）              |
| I2C            |     支持     | I2C0 / I2C1（硬件 I2C），I2C2（硬件 I2C，寄存器结构与 I2C0/1 不同） |
| SPI            |     支持     | SPI0                                                          |
| GPIO 中断      |     支持     | 支持外部中断功能                                                |
| **扩展模块**   | **支持情况** | **备注**                                                      |
| I2C EEPROM     |     支持     | AT24C02，使用 I2C0 总线                                        |
| SPI Flash      |     支持     | 使用 SPI0 总线                                                 |
...
- SPI Flash 测试功能需要外接或板载 SPI Flash 模块，如果开发板未配备 SPI Flash，请在 main.c 中注释掉 `#define GD32_SPI_TEST`
- I2C EEPROM 使用 I2C0（hwi2c0）总线
- SPI Flash 使用 SPI0 总线
- GD32E50x 系列 I2C2 与 I2C0/I2C1 的寄存器结构不同（I2C0/I2C1 属于同一类"legacy" IP，I2C2 单独属于另一类"new" IP），驱动层（drv_hard_i2c.c）已按此差异适配
- 串口默认使用 UART1（115200-8-1-N）
