...
#define LED1_PIN    GET_PIN(C, 7)   /* LED1 on PC7 */
#define LED2_PIN    GET_PIN(C, 8)   /* LED2 on PC8 */
#define LED3_PIN    GET_PIN(C, 9)   /* LED3 on PC9 */
#define LED4_PIN    GET_PIN(C, 11)  /* LED4 on PC11 */
...
static void spi_sample(void)
{
    ...
    rt_spi_bus_attach_device_cspin(spi_dev, SPI_NAME, BUS_NAME, GET_PIN(D, 2), RT_NULL);
