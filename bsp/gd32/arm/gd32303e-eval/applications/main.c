 ...
 * 2026-04-29     RT-Thread    add DMA test cases for new driver framework
 */
...
/* LED pins - GD32303E-EVAL board
 * LED1 = power indicator (no GPIO)
 * LED2 = PF0, LED3 = PF1, LED4 = PF2, LED5 = PF3
 */
#define LED2_PIN    GET_PIN(F, 0)
#define LED3_PIN    GET_PIN(F, 1)
#define LED4_PIN    GET_PIN(F, 2)
#define LED5_PIN    GET_PIN(F, 3)

#ifdef GD32_GPIO_EXTI_TEST
/* K1 = PA0 (Wakeup), K2 = PC13 (Tamper) */
...
/* SPI0: SCK=PB13, MISO=PB14, MOSI=PB15, CS=PB12 */
...
/* UART1 (PA2/PA3) is the default console, use it for interrupt RX echo test */
...
/*
 * UART DMA test: DMA RX + DMA TX echo on uart2 (PB10/PB11)
 * NOTE: uart2 (PB10/PB11) conflicts with I2C1, so use uart2 instead.
 */
...
#ifdef GD32_SPI_POLL_TEST
/**
 * @brief SPI Flash polling mode test - transfer < 16 bytes
 */
static void spi_poll_sample(void)
{
...
#ifdef GD32_SPI_DMA_TEST
/**
 * @brief SPI Flash DMA mode test - transfer >= 16 bytes
 */
static void spi_dma_sample(void)
{
#ifndef BSP_SPI0_USING_DMA
    rt_kprintf("[DMA] Warning: BSP_SPI0_USING_DMA not enabled!\n\r");
    rt_kprintf("[DMA] Test will use polling mode instead.\n\r");
    rt_kprintf("[DMA] Enable 'Enable SPI0 DMA' in menuconfig to use DMA.\n\r\n\r");
#endif
 ...
 * @param argv argument vector, argv[1] = uart name (optional, default "uart0")
...
{
    rt_kprintf("Wakeup key (K1) pressed!\n\r");
}

void tamper_key_pin_cb(void *args)
{
    rt_kprintf("Tamper key (K2) pressed!\n\r");
}
...
static void pin_irq_sample(void)
{
    ...
    rt_kprintf("GPIO EXTI ready. Press K1(PA0) or K2(PC13) key.\n\r");
}
#endif
