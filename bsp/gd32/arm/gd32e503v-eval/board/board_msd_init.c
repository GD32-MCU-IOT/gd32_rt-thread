 ...
 * Change Logs:
 * Date           Author       Notes
 * 2026-08-12     RT-Thread    first implementation for GD32E503V-EVAL
 */
...
#ifdef RT_USING_I2C
/* GD32E50x I2C2 shares the "new IP" register layout with GD32E51x, so this bus is
 * excluded from the generic AF-based GPIO table and needs an explicit init here. */
...
#ifdef RT_USING_SPI
/* GD32E50x has no gpio_af_set()/gpio_mode_set() API, so SPI is excluded from the
 * generic driver's GPIO table and needs an explicit init here. */
...
/* GD32E50x has no gpio_af_set()/gpio_mode_set() API, so UART is excluded from the
 * generic driver's GPIO table and needs an explicit init here. */
