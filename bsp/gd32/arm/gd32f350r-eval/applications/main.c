...
/* Note: The f350r eval board not support SPI flash, if you want to test this function,
...
#define LED1_PIN    GET_PIN(C, 10)   /* LED1 on PC10 */
#define LED2_PIN    GET_PIN(C, 11)   /* LED2 on PC11 */
#define LED3_PIN    GET_PIN(C, 12)   /* LED3 on PC12 */
#define LED4_PIN    GET_PIN(C, 2)    /* LED4 on PC2 */
...
static int uart_sample(int argc, char *argv[])
{
    ...
    rt_thread_t thread = rt_thread_create("serial", serial_thread_entry, RT_NULL, 512, 25, 10);
